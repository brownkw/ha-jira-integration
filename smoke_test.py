...
async def main() -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if issues:
            sample = issues[0]
            key = sample.get("key", "?")
            summary = sample["fields"].get("summary", "?")[:50]
            status = sample["fields"].get("status", {}).get("name", "?")
            priority = (sample["fields"].get("priority") or {}).get("name", "?")
            project = sample["fields"].get("project", {}).get("key", "?")
        ...
        print(f"   ✅  {len(fields)} custom field(s) available")
        ...
        options = {CONF_PRIORITIES: DEFAULT_HIGH_PRIORITY_NAMES}
        result = aggregate(issues, options)

        print(f"   Total open:      {result['total_open']}")
        print(f"   Overdue:         {result['overdue']}")
        print(f"   Due within 3d:  {result['due_soon']}")
