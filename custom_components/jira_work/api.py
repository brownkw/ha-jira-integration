...
class JiraClient:
    ...
    async def get_custom_fields(
        ...
        """Return {customfield_id: display_label} for custom fields.

        If *issues* is provided, only fields that have a non-null value on at
        least one of those issues are returned.  This keeps the config-flow
        dropdown manageable (tens of fields instead of tens of thousands).
        ...
        # If issues supplied (and non-empty), restrict to fields populated on
        # those issues. If the list is empty, fall back to all custom fields
        # so the Options dropdown isn't blank for users with no open issues.
        if issues:
            ...
            for issue in issues:
                for key, val in issue.get("fields", {}).items():
                    if key.startswith("customfield_") and val is not None:
                        populated.add(key)
            custom = [f for f in custom if f["id"] in populated]
